...
module.exports = function(app) {
    ...
    .then(function() {
      res.redirect(307, "/api/login.html");
    }).catch(function(err) {
      console.log(err);
      res.json(err);
...
//////
    ...
    app.get("/api/users/:id/ratings", (req,res)=>{
        db.Ratings.findById(req.params.id)
        .then(user=>{
            user.getRatings()
            .then(ratings=>{
                res.json(ratings);
...
//////
    ...
    app.post("/api/users/assignroles", function(req,res){
        ...
        .then(user=>{
            user.addRole(req.body.loadIds);
        });
    });


    app.post("/api/user/assignloads", function(req,res){
        db.User.findById(req.body.userId)
        .then(user=>{
            user.addLoads(req.body.loadId);
        });
    });
    //takes an object {userId, [roleIds]}
